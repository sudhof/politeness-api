...
DEPENDENCY_RE = re.compile(r"""([a-zA-Z0-9!?_]+[\(][0-9a-zA-Z!?\s,-]+[\)])[,\]]""")
...
class StanfordDependencyParser:
	...
	ROOT = DIR
	COMMAND = 'java -mx3000m -cp ".:./*" CsvParser $INPUT $COLUMN $OUTPUT'
	...
	def __read_csv(self, path):
		...
		for row in reader:
			...
			dependencies = DEPENDENCY_RE.findall(dependencies)
